...
// 暫定モックAPI（dist/routes/* が無い場合の表示用）
console.log('⚠ 暫定モックAPIを有効化します（フル表示用）');
const departments = [
  { id: 1, name: '撮影部', color: '#007bff' },
  { id: 2, name: '編集部', color: '#28a745' },
  { id: 3, name: '音響部', color: '#ffc107' }
];
const employees = [
  { id: 1, name: '田中太郎', departmentId: 1, position: 'カメラマン' },
  { id: 2, name: '佐藤花子', departmentId: 2, position: 'エディター' },
  { id: 3, name: '鈴木一郎', departmentId: 3, position: '音響技師' }
];
const equipment = [
  { id: 1, name: 'Aスタジオ', type: 'スタジオ', capacity: 10 },
  { id: 2, name: 'ライティングB', type: '照明', capacity: 5 },
  { id: 3, name: 'マイクセットC', type: '音響', capacity: 3 }
];
const schedules = [];
const templates = [
  { id: 1, name: '撮影テンプレート', title: '撮影作業', color: '#007bff' },
  { id: 2, name: '編集テンプレート', title: '編集作業', color: '#28a745' }
];
...
app.get('/api/schedules/daily-all', (req, res) => res.json([]));
app.get('/api/templates', (req, res) => res.json(templates));

// 静的ファイルの提供（APIルートの後に配置）
app.use(express.static(path.join(__dirname, 'suke', 'build')));

// SPA のルーティング対応（/api を除外）
app.get(/^\/(?!api\/).*/, (req, res) => {
  res.sendFile(path.join(__dirname, 'suke', 'build', 'index.html'));
});
