...
const req = http.request(options, (res) => {
  ...
  res.on('end', () => {
    ...
    db.all('SELECT * FROM equipment_reservations WHERE start_datetime LIKE "2025-09-24%" ORDER BY id DESC', (err, rows) => {
      if (err) {
        ...
        rows.forEach((row, index) => {
          console.log(`${index + 1}. ID: ${row.id} - ${row.purpose} (${row.start_datetime})`);
        });
      }
      db.close();
    });
